...
export function PagarTab({
  ...
  const pago = data.filter((r) => r.status === "quitada").reduce((s, r) => s + r.amount, 0);
  const vencidas = data.filter((r) => r.status === "atrasada").length;
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <KPI
          label="Vencidas"
          value={`${vencidas} conta${vencidas !== 1 ? "s" : ""}`}
          tone={vencidas > 0 ? "red" : "green"}
          icon="🚨"
        />
      </div>
      ...
      {paying && (
        <div className="rounded-2xl border border-rose-500/40 bg-rose-500/10 p-5">
          <div className="mb-3 font-semibold text-rose-300">
            Registrar pagamento: {paying.description}
          </div>
          {error && (
          ...
          <div className="mt-4 flex gap-3">
            <Btn tone="red" onClick={confirmPay} disabled={saving}>
      ...
      <Section title="Contas a Pagar" tone="red">
        <Table
          ...
          rows={data}
          cols={[
            { key: "counterparty", label: "Credor", render: (v) => String(v || "-") },
                ...
                row.status !== "quitada" && row.status !== "cancelada" ? (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="rounded px-2 py-1 text-[11px] font-semibold text-rose-400 hover:bg-rose-500/10"
                    >
                      Pagar
                    </button>
                    <button
